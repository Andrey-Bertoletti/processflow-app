...
export default function CampaignsPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (error) console.error("[CAMPAIGN_DEL_ERR]", error);
    else load();
  };
  ...
  return (
    <main className="app-shell p-8">
      <header className="app-enter mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Button onClick={() => { setEdit(null); setIsOpen(true); }} className="h-fit">
          ✨ Nova Campanha
        </Button>
      </header>
      ...
      {isLoading ? (
        ...
        <Surface className="p-12 text-center border-dashed">
          ...
          <Button variant="secondary" className="mt-6" onClick={() => setIsOpen(true)}>
            Começar Agora
          </Button>
        </Surface>
      ) : (
            ...
            <Surface key={c.id} className="group p-6 flex flex-col hover:border-blue-500/30 transition-all duration-300">
              ...
              <div className="flex gap-2 mt-6 pt-4 border-t border-slate-800/50">
                <Button variant="secondary" className="flex-1 text-xs"
                    onClick={() => { setEdit(c); setIsOpen(true); }}>
                  Editar
                </Button>
                <Button variant="danger" className="px-4 text-xs"
